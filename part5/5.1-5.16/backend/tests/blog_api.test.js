...
describe("when there is initially some blogs saved", () => {
  ...
  describe("viewing a specific blog", () => {
    test("succeeds with a valid id", async () => {
      ...
      const blogToView = blogsAtStart[0].id;
      const resultBlog = await api
        .get(`/api/blogs/${blogToView}`)
        .expect(200)
        .expect("Content-Type", /application\/json/);
      assert.deepStrictEqual(resultBlog.body.title, blogsAtStart[0].title);
    });
  ...
  describe("upgrading a blog", () => {
    test("blog can be upgraded", async () => {
      const blogsAtStart = await helper.blogsInDb();
      const token = await helper.userToken();
      const id = String(blogsAtStart[0].id);
      const blogToUpgrade = {
        title: "Upgraded Title",
        url: "http://sdsdsddssd.co",
        author: "elauthoraso",
      };
      await api
        .put(`/api/blogs/${id}`)
        ...
        .send(blogToUpgrade)
        .expect(200);
      const blogsAtENd = await helper.blogsById(id);
      assert.strictEqual(blogsAtENd.title, "Upgraded Title");
    });
  });
